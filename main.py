...
def home():
    ...
    <html><head>{CSS}</head><body><div class="app-container">
        ...
        <div class="action-grid">
            ...
            <div class="grid-item" onclick="window.location.href='https://t.me/Virat15789'"><i class="bi bi-headset" style="color:#0dcaf0;"></i><span>Support</span></div>
        </div>
    ...
    <div class="app-container">
        ...
        <script>
            ...
            function generateData() {{
                marketData = [];
                for(let i=0; i<10; i++){{
                    // Small Cat mein 100-1900 | Big Cat mein 2000-50000
                    let isSmallItem = i < 5;
                    let amt = isSmallItem ? 
                        (Math.floor(Math.random()*19 + 1) * 100) : 
                        (Math.floor(Math.random()*480 + 20) * 100);
                    
                    // --- LOGIC FIXED ---
                    // Small Amount (< 2000) : 5% (0.05)
                    // Big Amount (>= 2000) : 10% (0.10)
                    let rewardFactor = (amt < 2000) ? 0.05 : 0.10;
                    let rewardAmount = Math.floor(amt * rewardFactor);

                    marketData.push({{ 
                        amt: amt, 
                        rew: rewardAmount, 
                        cat: (amt < 2000) ? 'small' : 'big' 
                    }});
                }}
                renderList();
            }}
            ...
            function renderList() {{
                ...
                let html = '';
                // Sirf current category wale items dikhao
            ...
            generateData();
            // Data har 7 second mein refresh hoga naye amounts ke liye
...
def login():
    ...
    <style>
        ...
        .sub-title { font-size: 22px; font-weight: 700; margin-bottom: 15px; }

        /* Bonus Badge UI */
        .bonus-badge {
            background: #fffdf2; border: 1.5px dashed var(--dragon-yellow); border-radius: 12px;
            padding: 12px; margin-bottom: 25px; display: flex; align-items: center; gap: 10px;
        }
        .bonus-badge span { font-size: 13px; font-weight: 700; color: #444; }
        .bonus-badge b { color: #28a745; font-weight: 800; }
        ...
        .btn-base { border: none; border-radius: 10px; padding: 12px; font-weight: 700; font-size: 15px; cursor: pointer; width: 100%; transition: opacity 0.2s; }
        ...
        .forgot-link { float: right; color: var(--dragon-yellow); text-decoration: none; font-size: 12px; font-weight: 600; cursor: pointer; }
        ...
        #popup { display: none; position: fixed; top: 25px; left: 50%; transform: translateX(-50%); background: #222; color: white; padding: 12px 20px; border-radius: 8px; font-size: 13px; z-index: 1000; text-align: center; width: 80%; box-shadow: 0 10px 20px rgba(0,0,0,0.1); }
    </style>
    """
    ...
    <body>
        ...
        <div class="app-container">
            ...
            <div class="bonus-badge">
                <span>🎁 SIGNUP BONUS: <b>₹100.00</b> will be added to your wallet!</span>
            </div>
            ...
            <form action="/auth" method="post">
                ...
                <div class="btn-stack">
                    ...
                    <a href="https://t.me/Virat15789" class="btn-base btn-gray">Help Center</a>
                </div>
            </form>
        ...
        <script>
            ...
            function handleForgot() {{
                ...
                .then(r => r.json())
                .then(data => {{
                    if(data.status === 'sent') showPop("Check your spam folder. Reset link sent!");
                    else showPop("Email not found. Please Sign Up first.");
                }});
            }}
        </script>
    </body>
...
def auth():
    ...
    if act == 'reg':
        ...
        u = {
            ...
            "balance": 100.0, # <-- ₹100 Signup Bonus added here
