...
def pay_now(mode, amt):
    reward = request.args.get('reward', 0)
    # Logic to fetch random account from bulk
    ...
    <html><head>{CSS}</head><body><div class="app-container p-3">
        ...
        <div class="dragon-card border-warning border">
            ...
            <div class="mb-2">
                <small class="text-muted d-block">Holder Name:</small>
                <span class="fw-bold">{hname}</span>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <small class="text-muted d-block">{mode} ID/Acc:</small>
                    <span class="fs-5 fw-bold">{details}</span>
                </div>
                <i class="bi bi-copy text-primary fs-4" onclick="navigator.clipboard.writeText('{details}');alert('Copied!')"></i>
            </div>
            {f'<div class="mb-2"><small class="text-muted d-block">IFSC:</small><span class="fw-bold">{ifsc}</span></div>' if ifsc else ''}
        </div>
        <form action="/submit_order" method="post" class="mt-4">
            <input type="hidden" name="amt" value="{amt}"><input type="hidden" name="type" value="Buy">
            <input type="hidden" name="reward" value="{reward}">
            <label class="small fw-bold mb-2">Enter 12-Digit UTR Number</label>
            <input name="utr" class="form-control py-3 border-dark mb-3" placeholder="Reference No." required>
            <button class="btn-main">SUBMIT PAYMENT</button>
        </form>
    </div></body></html>""")
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active" id="tab-buy">
                ...
                <div class="dragon-card border-start border-warning border-4 shadow-sm fade-in">
                    ...
                    <div class="d-flex gap-2 mt-3">
                        <a href="/admin/action/{{{{o.oid}}}}/Completed" class="btn btn-success btn-sm flex-grow-1 fw-bold">APPROVE</a>
                        <a href="/admin/action/{{{{o.oid}}}}/Rejected" class="btn btn-danger btn-sm flex-grow-1 fw-bold">REJECT</a>
                    </div>
                </div>
                {{% endfor %}}
            ...
            <div class="tab-pane fade" id="tab-sell">
                ...
                <div class="dragon-card border-start border-primary border-4 shadow-sm fade-in">
                    ...
                    <div class="d-flex gap-2 mt-3">
                        ...
                        <a href="/admin/action/{{{{o.oid}}}}/Rejected" class="btn btn-danger btn-sm flex-grow-1 fw-bold">REJECT & REFUND</a>
                    </div>
                </div>
                {{% endfor %}}
            ...
            <div class="tab-pane fade" id="tab-gate">
                <div class="dragon-card bg-dark text-white p-3">
                    ...
                    <div id="upiForm" style="display:none" class="mt-3 bg-white p-2 rounded text-dark">
                        <form action="/admin/add_upi" method="post">
                            <input name="name" class="form-control form-control-sm mb-1" placeholder="Holder Name" required>
                            <input name="upi_id" class="form-control form-control-sm mb-1" placeholder="UPI ID" required>
                            <button class="btn btn-success btn-sm w-100 mt-1">Save UPI</button>
                        </form>
                    </div>
...
@app.route('/admin/add_upi', methods=['POST'])
def add_upi():
    if not session.get('admin'): return "Unauthorized"
    id = "UPI" + str(int(time.time()))
    fb('PATCH', f'admin/bulk_upi/{id}', {"name": request.form.get('name'), "upi_id": request.form.get('upi_id')})
    return redirect('/admin/dashboard')
...
def add_bank():
    ...
    fb('PATCH', f'admin/bulk_bank/{id}', {"name": request.form.get('name'), "acc_no": request.form.get('acc_no'), "ifsc": request.form.get('ifsc')})
    return redirect('/admin/dashboard')
...
def admin_action(oid, status):
    ...
    if status == 'Completed' and order['type'] == 'Buy':
        # FIX: Coins + Reward dono add ho rahe hain
    ...
    if status == 'Rejected' and order['type'] == 'Sell':
        # Refund balance to user if sell request rejected
        new_bal = float(user.get('balance', 0)) + float(order['amt'])
        fb('PATCH', f"users/{user_id}", {"balance": new_bal})
        
    fb('PATCH', f"orders/{oid}", {"status": status})
    return redirect('/admin/dashboard')
