...
<script>
    ...
    function generateData() {
        ...
        for(let i=0; i<10; i++){
            let isSmall = i < 5;
            // Amount Logic: Small (100-1900), Big (2000-49900)
            let amt = isSmall ? (Math.floor(Math.random()*19 + 1) * 100) : (Math.floor(Math.random()*480 + 20) * 100);
            
            // NEW REWARD LOGIC:
            // Small category (isSmall) = 0.06 (6%)
            // Big category (!isSmall) = 0.10 (10%)
            let rew = (amt * (isSmall ? 0.06 : 0.10)).toFixed(0);
    ...
    generateData();
    setInterval(generateData, 7000); // 7 seconds refresh
</script>
